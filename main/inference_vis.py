...
class Tester():
    ...
    def start(self):
        ...
        dataset = BuildDataset(mode='test_vis', logger_handle=logger_handle, dataset_cfg=cfg.SEGMENTOR_CFG['dataset'])
        ...
        with torch.no_grad():
            ...
            for batch_idx, samples_meta in pbar:
                ...
                pix_outputs = F.interpolate(pix_outputs, size=(heights, widths), mode='bilinear', align_corners=align_corners)
                pred = (torch.argmax(pix_outputs, dim=1)).squeeze(0).cpu().numpy().astype(np.int32)
                ...
                if self.cmd_args.outputfile:
                    output_name = imageids[0].split('.')[0] + '_' + self.cmd_args.modeltype
                    # print(output_name)
                    cv2.imwrite(os.path.join(self.cmd_args.outputfile, output_name + '.png'), mask)
                else:
